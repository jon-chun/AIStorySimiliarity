    ...
    for file in sorted(os.listdir(subdir_path)):
        if file.endswith('.json'):
            ...
            if all((ref_film, test_film, element_type, version)):
                key = (ref_film, test_film)
                if files_processed[key] >= MAX_SCRIPT_CT:
                    continue
                ...
                if data:
                    ...
                    if files_processed[key] < len(all_data):
                        # Update existing row
                        all_data[files_processed[key]].update({f'{element_type}-{k}': v for k, v in data.items()})
                    else:
                        # Create new row
                        row = {
