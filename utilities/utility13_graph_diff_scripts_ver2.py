...
SCRIPT_SOURCE = 'Script' # Choose from 'GenAI', 'Elements', 'Script'
cwd = os.getcwd()
INPUT_ROOT_DIR = os.path.abspath(os.path.join(cwd, '..', 'data', 'score_diff_scripts_summary'))
OUTPUT_ROOT_DIR = os.path.abspath(os.path.join(cwd, '..', 'data', 'figures_diff_scripts'))
...
def compute_similarity_mean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available_columns = [col for col in overall_columns if col in df.columns]
    if available_columns:
        df['similarity-mean'] = df[available_columns].mean(axis=1)
    else:
        print(f"Warning: No overall columns found in {df.columns}")
    return df
...
def create_bar_chart(data: Dict[str, pd.DataFrame], element: str, title: str, filename: str):
    ...
    if element == 'overall':
        bar_data = {film: df['similarity-mean'].mean() if 'similarity-mean' in df.columns else np.nan for film, df in data.items()}
    else:
        bar_data = {film: df[f'{element}-overall'].mean() if f'{element}-overall' in df.columns else np.nan for film, df in data.items()}
    
    # Remove any NaN values
    bar_data = {k: v for k, v in bar_data.items() if not np.isnan(v)}
    ...
    plt.title(f"[Scripts] {title}", fontsize=16)
...
def create_parallel_coordinates(data: Dict[str, pd.DataFrame], title: str, filename: str):
    elements = ['characters-overall', 'plot-overall', 'setting-overall', 'themes-overall']
...
def main():
    ...
    for filename in os.listdir(INPUT_ROOT_DIR):
        if filename.endswith('.csv'):
            ...
            print(f"Processing {filename}: {test_film}")
            print(f"Columns: {df.columns}")
            df = compute_similarity_mean(df)
            all_data[f"{test_film}"] = df
    
    print(f"Processed data for: {list(all_data.keys())}")
    
    # Save the combined dataframe
    combined_df = pd.concat(all_data.values(), keys=all_data.keys())
    combined_df.to_csv(os.path.join(OUTPUT_ROOT_DIR, "summary_diff_all_scripts.csv"))
