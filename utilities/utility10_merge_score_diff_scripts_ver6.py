    ...
    for file in sorted(os.listdir(subdir_path)):
        if file.endswith('.json'):
            ...
            if all((ref_film, test_film, element_type, version)):
                key = (ref_film, test_film, int(version))
                if files_processed[key] >= len(ELEMENT_TYPES):
                    logging.info(f"Skipping {file} as all element types for this version have been processed")
                    continue
                ...
                if data:
                    ...
                    row = next((d for d in all_data if d['reference_film'] == ref_film and d['test_film'] == test_film and d['version_number'] == version_number), None)
                    ...
                    if row is None:
                        ...
                        all_data.append(row)
                        logging.info(f"Created new row for {ref_film}, {test_film}, version {version_number}")
                    else:
                        logging.info(f"Updating existing row for {ref_film}, {test_film}, version {version_number}")
                ...
                logging.warning(f"Could not extract film info from filename: {file}")
