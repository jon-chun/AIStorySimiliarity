...
cwd = os.getcwd()
dir_name = 'verify_gpt4o' # pick from 'verify_claude35sonnet' or 'verify_gpt4o'
subdir_name = 'verify_gpt4o_facts-reasoning' # pick from 'verify_claude35sonnet_facts-reasoning' or 'verify_gpt4o_facts-reasoning'

dir_name = 'verify_claude35sonnet' 
subdir_name = 'verify_claude35sonnet_facts-reasoning' 
...
def merge_verify_files(input_root_dir, output_root_dir):
    ...
    pattern = re.compile(r"verify_(?P<model_name>.+)_facts-reasoning_(?P<test_film>.+)_ver(?P<version_number>\d+)\.json")
    ...
    for (model_name, test_film), dfs in grouped_data.items():
        ...
        output_filename = f"summary_verify_facts-reasoning_{model_name}_{test_film}.csv"
