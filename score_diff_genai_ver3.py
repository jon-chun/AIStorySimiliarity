...
# Use this function in the call_openai function
...
# Use this function instead of read_file where appropriate
...
def get_latest_version_and_film(output_dir: str) -> Tuple[int, Optional[str]]:
    ...
    for file in reversed(files):
        if file.startswith("similarity-by-score_") and file.endswith(".pkl"):
            parts = file.split("_")
            if len(parts) >= 4:
                try:
                    version = int(parts[-1].split(".")[0][3:])
                    if version > latest_version:
                        latest_version = version
                        last_film = "_".join(parts[2:-1])
                        break
                except ValueError:
                    continue
...
def main():
    ...
    for sample in range(start_version, start_version + SAMPLE_SIZE):
        ...
        for film_index, (film_name, film_year) in enumerate(scripts_list[start_film_index:], start=start_film_index):
            output_file = os.path.join(OUTPUT_DIR_SIM_BY_SCORE, version_filename(f"similarity-by-score_{COMPARISON_TYPE}_{SCRIPT_REFERENCE}_{film_name}", sample))

            if os.path.exists(f"{output_file}.csv") and not OVERWRITE_FLAG:
                print(f"Skipping existing file for film {film_name} (Version {sample})")
                continue
            ...
            try:
                ...
                # Save results after each film is processed
                try:
                    pkl_output_file = f"{output_file}.pkl"
                    save_to_pkl(rankbyscore_refgentestgen_filmdict, pkl_output_file)
    ...
    if not rankbyscore_refgentestgen_filmdf.empty:
        ...
        try:
            final_datetime_str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
            final_pkl_output = version_filename(f"{output_fullpath_pkl.rsplit('.', 1)[0]}_{final_datetime_str}", sample)
            save_to_pkl(rankbyscore_refgentestgen_filmdict, final_pkl_output)
            save_to_csv(rankbyscore_refgentestgen_filmdf, version_filename(output_fullpath_csv, sample))
