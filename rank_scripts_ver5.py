...
if COMPARISON_TYPE == 'genai-genai':
    ...
    output_fullpath_csv = os.path.join(OUTPUT_DIR_SIM_BY_SCORE, output_filename_csv)
    INPUT_DIR_SIM_BY_SCORE = os.path.join('data','film_narrative_elements')
...
def get_element_description(film_name, element_type):
    print(f"IN get_element_description(): film_name: {film_name}, element_type: {element_type}")
    film_elements_subdir_list = sorted(os.listdir(INPUT_DIR_SIM_BY_SCORE))
    for film_element_subdir in film_elements_subdir_list:
        if film_name in film_element_subdir:
            print(f"reference_element in film_element_subdir")
        elif film_name in film_element_subdir:
            print(f"test_elements in film_element_subdir")
        else:
            continue
    return

def get_element_distance(script_reference, script_test, narrative_element, unique_id):

    print(f"IN get_element_distance(): script_reference: {script_reference}, script_test: {script_test}, narrative_element: {narrative_element}")
    title_clean_reference = clean_title(SCRIPT_REFERENCE)
    title_clean_test = clean_title(script_test)
    ...
    test_element_description = get_element_description(script_test, narrative_element)
