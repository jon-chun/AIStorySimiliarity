...
def compare_films(elements_all_films_dict: Dict, reference_key: str, test_key: str, version: int) -> Dict:
    try:
        ...
        base_filename = f"compare_elements_{reference_key}_{test_key}.csv"
        individual_output_file = os.path.join(OUTPUT_ROOT_DIR, version_filename(base_filename, version))
...
def output_file_exists(reference_key: str, test_key: str, version: int) -> bool:
    """Check if the output file for a specific comparison already exists."""
    base_filename = f"compare_elements_{reference_key}_{test_key}.csv"
...
def compare_films(elements_all_films_dict: Dict, reference_key: str, test_key: str, version: int) -> Optional[Dict]:
    if output_file_exists(reference_key, test_key, version):
...
def load_existing_comparisons(version: int) -> List[Dict]:
    overall_output_file = os.path.join(OUTPUT_ROOT_DIR, version_filename('element_distances_all.csv', version))
    if os.path.exists(overall_output_file):
        return pd.read_csv(overall_output_file).to_dict('records')
    return []
""";
...
def main():
    ...
    for version in range(SAMPLE_SIZE):
...
def main():
    ...
    for version in range(SAMPLE_SIZE):
        ...
        if element_distances_all:
            ...
            print(f"Overall element distances (Version {version}) saved to {overall_output_file}")
        else:
            log_error(f"No valid comparisons were made in version {version}")
